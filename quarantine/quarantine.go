...
func Flaky(tb testing.TB, ticket string) {
	...
	if os.Getenv("RUN_FLAKY_TESTS") != "true" {
		tb.Skipf(
			"Skipping %s. To run flaky tests, set the RUN_FLAKY_TESTS environment variable to true.",
			explanationStr,
		)
	} else {
		tb.Logf("Running %s", explanationStr)
		tb.Cleanup(func() {
			tb.Logf("Test is marked as flaky, but still ran. %s. To skip flaky tests, ensure the RUN_FLAKY_TESTS environment variable is set to false.", explanationStr)
		})
	}
}
